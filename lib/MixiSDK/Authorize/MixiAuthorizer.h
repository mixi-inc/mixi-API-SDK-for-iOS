...
#define kCurrentPermissions [NSArray array]
...
@interface MixiAuthorizer : NSObject {
    ...
    /** \brief 認可解除後に自動的にログアウトするかどうか */
    BOOL shouldLogoutAfterRevoke_;
...
@property (nonatomic, assign) BOOL shouldLogoutAfterRevoke;
 ...
 * \brief 認可を実行
 *
 * \parem parentViewController 親ビューコントローラー（mixi公式iPhoneアプリがインストールされている場合は無視されます）
...
/**
 * \brief 有効なパーミッションかどうかを確認し、有効なら保持
 *
 * \param permissions 要求するパーミッション
 * \throw パーミッションが不正
 * \return パーミッションが有効ならYESを返します。
 */
