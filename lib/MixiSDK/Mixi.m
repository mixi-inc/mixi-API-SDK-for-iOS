...
- (id)setupWithConfig:(MixiConfig*)config {
    self.config = config;
    MixiMultiplexAuthorizer *authorizer = [MixiMultiplexAuthorizer authorizer];
    if (self.config.redirectUrl) authorizer.sdkAuthorizer.redirectUrl = self.config.redirectUrl;
...
- (NSString*)application:(UIApplication *)application openURL:(NSURL *)url sourceApplication:(NSString *)sourceApplication annotation:(id)annotation error:(NSError**)error 
{
    if ([[url path] isEqualToString:@"/error"]) {
        ...
        if (pasteboard == nil) {
            ...
            [UIPasteboard removePasteboardWithName:self.config.pbkey];
    ...
    else if ([[url host] isEqualToString:@"revoke"]) {
        if (self.authorizer.shouldLogoutAfterRevoke) [self logout];
        return kMixiAppApiTypeRevoke;
    }
...
- (BOOL)logoutAndRevoke {
    return [self revokeAndLogout];
}

- (BOOL)revokeAndLogout {
    return [self.authorizer revokeAndLogout];
}

- (BOOL)logoutAndRevokeWithError:(NSError**)error {
    return [self revokeAndLogoutWithError:error];
}

- (BOOL)revokeAndLogoutWithError:(NSError**)error {
    return [self.authorizer revokeAndLogoutWithError:(NSError**)error];
}
...
- (NSURLConnection*)sendRequest:(MixiURLRequestConstructor*)request delegate:(id<MixiDelegate>)delegate forced:(BOOL)forced {
    BOOL shouldNotifyError = delegate != nil && [delegate respondsToSelector:@selector(mixi:didFailWithError:)];
    ...
    if (!forced && [self.authorizer isAccessTokenExpired]) {
        NSError *error = nil;
        if (![self.authorizer isRefreshTokenExpired] && self.autoRefreshToken) {
            [self.authorizer refreshAccessTokenWithError:&error];
            if (!error) {
                [self store];
        ...
        if (error && error.domain == kMixiErrorDomain && error.code == kMixiAPIErrorRefreshTokenExpired
            && request.openMixiAppToAuthorizeIfNeeded) {
            if (![self authorizeForPermissions:kCurrentPermissions]) {
                if (shouldNotifyError) {
                    error = [NSError errorWithDomain:kMixiErrorDomain
                                                code:kMixiTokenErrorOpenAuthorizer
            ...
            if (!error || error.domain != kMixiErrorDomain) {
                error = [NSError errorWithDomain:kMixiErrorDomain
...
- (NSDictionary*)sendSynchronousRequest:(MixiURLRequestConstructor*)request error:(NSError**)error {
    ...
    if ([request isKindOfClass:[MixiBatchRequest class]]) {
        return [NSDictionary dictionaryWithObject:json forKey:@"batched_response"];
    }
    else if ([json respondsToSelector:@selector(objectForKey:)]) {
        ...
        if (error != nil) {
            *error = [NSError errorWithDomain:kMixiErrorDomain
                                         code:kMixiAPIErrorInvalidJson
                                     userInfo:[NSDictionary dictionaryWithObject:json forKey:@"json"]];
        }
        return nil;
    }
}
