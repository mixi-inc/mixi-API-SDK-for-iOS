...
- (void)notifyCancel;
/* webViewにhtmlを表示 */
- (void)loadHtmlString:(NSString*)html mixi:(Mixi*)mixi;
@end
/** \endcond */
...
- (void)openURL:(NSURL*)url {
    ...
    if ([result hasPrefix:@"<!DOCTYPE html>"]) {
        [self loadHtmlString:result mixi:mixi_];
        return;
    }
...
- (void)mixi:(Mixi*)mixi didFinishLoading:(NSString*)data {
    [self loadHtmlString:data mixi:mixi];
...
- (void)loadHtmlString:(NSString*)html mixi:(Mixi*)mixi {
    NSString *htmlString = [html stringByReplacingOccurrencesOfString:@"__MIXI_URL_SCHEME__" withString:mixi.config.urlScheme];
